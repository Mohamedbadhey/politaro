...
function saveDraft() {
    ...
    if (currentCaseData && currentCaseData.id) {
        saveMedicalFormToDatabase(savedForm);
    }
...
function generateQRCode() {
    ...
    const reportDate = document.querySelector('input[name="report_date"]')?.value || new Date().toISOString().split('T')[0];
    
    // Generate QR code data
    const qrData = `MEDICAL-REPORT:${caseNumber}:${reportDate}:${Date.now()}`;
