    ...
    try {
        ...
        if (response.status === 'success') {
            ...
            if (role === 'investigator') {
                content.html(renderInvestigatorDashboard(data));
...
async function loadCasePersonsPage() {
    ...
    const content = $('#pageContent');
    content.html('<div class="loading">Loading persons...</div>');
    ...
    try {
        ...
        content.html('<div class="alert alert-error">Failed to load case persons: ' + error.message + '</div>');
    }
}
...
function renderCasePersonsPage(persons) {
    ...
    content.html(`
        <div class="case-persons-container">
            <div class="persons-header">
                <h1><i class="fas fa-user-friends"></i> Case Persons</h1>
                <p>All persons involved in your assigned cases</p>
            </div>
            
            <div class="filter-section">
                <div class="filter-group">
                    <input type="text" id="personSearch" placeholder="Search by name, ID, phone..." class="search-input">
                    ...
                    <select id="custodyFilter" class="filter-select">
            ...
            <div id="personsGrid" class="persons-grid">
                ${renderPersonsCards(persons)}
            </div>
        </div>
        
        <style>
            .case-persons-container { max-width: 1400px; margin: 0 auto; padding: 20px; }
            ...
            .persons-header h1 { margin: 0 0 10px 0; font-size: 32px; font-weight: 700; }
            .persons-header p { margin: 0; opacity: 0.95; }
            ...
            .person-info-header { flex: 1; }
            .person-name { margin: 0 0 5px 0; font-size: 18px; font-weight: 700; color: #111827; }
            .person-roles {
                display: flex;
                gap: 5px;
                flex-wrap: wrap;
            ...
            .role-accuser { background: #dbeafe; color: #2563eb; }
            .role-witness { background: #fef3c7; color: #d97706; }
            .role-other { background: #e5e7eb; color: #6b7280; }
            ...
            .custody-none { background: #e5e7eb; color: #6b7280; }
    ...
    $('#personSearch').on('input', filterPersons);
    $('#roleFilter').on('change', filterPersons);
    $('#custodyFilter').on('change', filterPersons);
}
    ...
    if (!persons || persons.length === 0) {
        return `
            <div style="grid-column: 1 / -1; text-align: center; padding: 60px; color: #6b7280;">
                ...
                <p>No persons are involved in your assigned cases yet.</p>
            </div>
        `;
    }
        ...
        html += `
            <div class="person-card" data-roles="${person.roles || ''}" data-custody="${custodyStatus}" data-name="${fullName.toLowerCase()}" data-id="${person.national_id || ''}" data-phone="${person.phone || ''}">
                <div class="person-card-header">
                    ...
                    <div class="person-info-header">
                        ...
                        <div class="person-roles">
                            ${roles.map(role => `<span class="role-badge role-${role.trim()}">${role.trim()}</span>`).join('')}
                        </div>
                    </div>
                </div>
                ...
                <div class="person-details">
                    ...
                    <div class="detail-item">
                        <span class="detail-label">Custody Status</span>
                        <span class="custody-badge custody-${custodyStatus}">
                            ...
                            ${custodyStatus === 'in_custody' ? 'In Custody' : custodyStatus === 'bailed' ? 'Bailed' : 'Not in Custody'}
                        </span>
                    </div>
                </div>
                ...
                <div class="person-stats">
                    <div class="stat">
                        ...
                        <div class="stat-label">Your Cases</div>
                    </div>
                    <div class="stat">
...
async function viewPersonFullDetails(personId) {
    try {
        ...
        closeAlert();
        await showError('Error', 'Failed to load person details: ' + error.message);
    }
}
...
function showPersonDetailsModal(person) {
            ...
            <div class="details-sections">
                <div class="details-section">
                    ...
                    <div class="info-grid">
                        ...
                        <div><strong>Date of Birth:</strong> ${person.date_of_birth || 'N/A'}</div>
                ...
                <div class="details-section">
                    <h3><i class="fas fa-briefcase"></i> Connected Cases (${cases.length})</h3>
                    ${cases.length > 0 ? `
                            ...
                            <tbody>
                                ${cases.map(c => `
                                    <tr>
                                        ...
                                        <td><span class="role-badge role-${c.party_role}">${c.party_role}</span></td>
                                        <td>${c.crime_type || 'N/A'}</td>
                                        <td>${getStatusBadge(c.status)}</td>
                                        <td><button class="btn btn-sm btn-primary" onclick="viewCase(${c.id})">View</button></td>
                                    </tr>
                                `).join('')}
                            </tbody>
                        </table>
                    ` : '<p style="text-align: center; color: #6b7280;">No cases found</p>'}
                </div>
                
                ${custodyRecords.length > 0 ? `
                    <div class="details-section">
                        <h3><i class="fas fa-lock"></i> Custody Records (${custodyRecords.length})</h3>
                        <table class="details-table">
                            <thead>
                                <tr>
                                    <th>Status</th>
                            ...
                            <tbody>
                                ${custodyRecords.map(cr => `
                                    <tr>
                                        ...
                                        <td>${cr.custody_end ? formatDateOnly(cr.custody_end) : '<em>Ongoing</em>'}</td>
                                    </tr>
                                `).join('')}
                            </tbody>
                        </table>
                ...
                ${criminalHistory.length > 0 ? `
                    <div class="details-section">
                        <h3><i class="fas fa-history"></i> Criminal History - Past Offenses (${criminalHistory.length})</h3>
                        <table class="details-table">
                            <thead>
                                <tr>
                                    <th>Case Number</th>
    ...
    showModal('Person Details', bodyHtml, [
    ...
    content.html(`
        <form id="obEntryForm" style="max-width: 1200px;">
            ...
            <div class="form-row">
                ...
                <div class="form-group">
                    ...
                    <select name="crime_category" required>
                        ...
                        <option value="drug" data-i18n="drug_related">${t('drug_related')}</option>
                        <option value="cybercrime" data-i18n="cybercrime">${t('cybercrime')}</option>
                        <option value="sexual" data-i18n="sexual_offense">${t('sexual_offense')}</option>
                        <option value="juvenile" data-i18n="juvenile">${t('juvenile')}</option>
            ...
            <div style="margin-top: 20px; display: flex; gap: 10px;">
                ...
                <button type="button" class="btn btn-success" onclick="submitOBEntry()"><span data-i18n="submit_for_approval">${t('submit_for_approval')}</span></button>
...
function addAccusedForm() {
    ...
    const formHtml = `
        <div class="party-form-section" data-party-index="${index}" style="margin-top: 20px; padding: 20px; border: 2px solid #e5e7eb; border-radius: 8px; position: relative;">
            ...
            <hr style="margin: 20px 0; border: none; border-top: 1px solid #e5e7eb;">
            <h5 style="color: #1f2937; margin-bottom: 15px;"><i class="fas fa-user-lock"></i> Custody Status</h5>
            ...
            <div id="arrested_fields_${index}" style="display: none;">
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input type="text" name="accused_custody_location_${index}" placeholder="e.g., Station Lock-up">
                    </div>
                    <div class="form-group">
                        <label>Cell Number</label>
                        <input type="text" name="accused_cell_number_${index}" placeholder="e.g., Cell 3">
                    </div>
                </div>
                
                <div class="form-group">
                    <label>Custody Notes</label>
                    <textarea name="accused_custody_notes_${index}" rows="2" placeholder="Any additional custody information..."></textarea>
                </div>
            </div>
            
            <!-- Bailed Fields -->
            <div id="bailed_fields_${index}" style="display: none;">
                <h6 style="color: #3b82f6; margin: 15px 0 10px 0;"><i class="fas fa-user-tie"></i> Person Who Bailed (Bailer Information)</h6>
                
                <div class="form-row">
                    <div class="form-group">
                        <label>Bailer Full Name *</label>
                        <input type="text" name="accused_bailer_name_${index}" placeholder="Full name of person who bailed">
                    </div>
                    <div class="form-group">
                        <label>Bailer National ID</label>
                        <input type="text" name="accused_bailer_id_${index}" placeholder="National ID number">
                    </div>
                </div>
                
                <div class="form-row">
                    <div class="form-group">
                        <label>Bailer Phone *</label>
                        <input type="tel" name="accused_bailer_phone_${index}" placeholder="Phone number">
                    </div>
                    <div class="form-group">
                        <label>Relationship to Accused</label>
                        <input type="text" name="accused_bailer_relationship_${index}" placeholder="e.g., Father, Brother, Friend">
                    </div>
                </div>
                
                <div class="form-group">
                    <label>Bailer Address</label>
                    <input type="text" name="accused_bailer_address_${index}" placeholder="Physical address of bailer">
                </div>
                
                <div class="form-group">
                    <label>Bail Conditions</label>
                    <textarea name="accused_bail_conditions_${index}" rows="2" placeholder="Enter bail conditions..."></textarea>
                </div>
                
                <div class="form-group">
                    <label>Bail Amount (if applicable)</label>
                    <input type="number" name="accused_bail_amount_${index}" placeholder="Amount in local currency" step="0.01">
                </div>
            </div>
        </div>
    `;
                ...
                cases.forEach(caseItem => {
                    html += `
                        <tr>
                            ...
                            <td>
                                <button class="btn btn-sm btn-secondary" onclick="viewCaseDetails(${caseItem.id})">View</button>
                                <button class="btn btn-sm btn-primary" onclick="editCase(${caseItem.id})">Edit</button>
                    ...
                    <tr>
                        <td colspan="9" style="text-align: center; padding: 40px;">
                            <div style="color: #6b7280;">
                                ...
                                <p style="margin: 10px 0;">There are currently no persons in custody at this station.</p>
                                <p style="margin: 10px 0; font-size: 14px;">
                                    Custody records are automatically created when you mark an accused as "Arrested" in the OB Entry page.
                                </p>
                            </div>
                        </td>
                    </tr>
            ...
            html += `
                <div class="bailer-card">
                    <div class="bailer-info">
                        ...
                        <div class="bailer-info-item">
                            <span class="bailer-info-label">National ID</span>
                            <span class="bailer-info-value">${bailer.national_id || 'N/A'}</span>
                        </div>
                        <div class="bailer-info-item">
                            <span class="bailer-info-label">Phone</span>
                            <span class="bailer-info-value">${bailer.phone || 'N/A'}</span>
                        </div>
                        <div class="bailer-info-item">
                            <span class="bailer-info-label">Address</span>
                            <span class="bailer-info-value">${bailer.address || 'N/A'}</span>
                        </div>
                        <div class="bailer-info-item">
                            <span class="bailer-info-label">Connected Cases</span>
                            <span class="bailer-info-value">${bailer.case_count || 0}</span>
                        </div>
                        <div class="bailer-info-item">
                            <span class="bailer-info-label">Persons Bailed</span>
                    ...
                    ${bailedPersons.length > 0 ? `
                        <div class="bailed-persons-list">
                            <h4 style="margin: 0 0 10px 0; font-size: 14px; color: #374151;">Persons Bailed:</h4>
                            ${bailedPersons.map(person => `
                                <div class="bailed-person-item">
                                    <div><strong>${person.bailed_person_name}</strong></div>
                                    <div style="font-size: 12px; color: #6b7280; margin-top: 5px;">
                                        Case: ${person.case_number} | Status: ${person.custody_status}
                                    </div>
                                </div>
                            `).join('')}
                        </div>
...
function loadPersonsPage() {
    ...
    content.html(`
        <div class="persons-page-container">
            <div class="persons-header">
                <h2><i class="fas fa-users"></i> Persons Management</h2>
                <p>View and manage all persons in the system</p>
            </div>
            ...
            <div class="persons-filters">
                <div class="filter-group">
                    ...
                    <select id="personTypeFilter" onchange="filterPersons()">
                        ...
                        <option value="accuser">Accuser</option>
                        <option value="witness">Witness</option>
                        <option value="other">Other (Bailer)</option>
                    </select>
                </div>
                <div class="filter-group">
                    <label>Search:</label>
                    <input type="text" id="personSearch" placeholder="Search by name, ID, phone..." onkeyup="filterPersons()">
                </div>
            </div>
...
async function loadPersonsTable() {
    try {
        ...
        $('#personsTableContainer').html('<div class="alert alert-error">Failed to load persons: ' + error.message + '</div>');
    }
}
...
function renderPersonsTable(persons) {
    ...
    if (persons.length === 0) {
        html += '<tr><td colspan="8" style="text-align: center; padding: 40px;">No persons found</td></tr>';
    } else {
        persons.forEach(person => {
            const photoUrl = person.photo_path ? `/${person.photo_path}` : '/assets/images/default-avatar.png';
            const fullName = `${person.first_name} ${person.middle_name || ''} ${person.last_name}`.trim();
...
function getCustodyStatusBadge(status) {
    const badges = {
        'in_custody': '<span class="badge badge-danger">In Custody</span>',
        'bailed': '<span class="badge badge-warning">Bailed</span>',
        'released': '<span class="badge badge-success">Released</span>',
        'transferred': '<span class="badge badge-info">Transferred</span>',
        'escaped': '<span class="badge badge-danger">Escaped</span>',
        'none': '<span class="badge badge-secondary">No Custody</span>',
        'null': '<span class="badge badge-secondary">No Custody</span>'
    };
                ...
                html: `
                    <div class="person-details-modal">
                        ...
                        <div class="person-info-grid">
                            ...
                            <div class="info-item">
                                <strong>National ID:</strong> ${person.national_id || 'N/A'}
                            </div>
                            <div class="info-item">
                            ...
                            <div class="info-item">
                                <strong>Date of Birth:</strong> ${person.date_of_birth || 'N/A'}
                            </div>
                        ...
                        <div class="person-cases-section">
                            ...
                            <div id="personCasesList">Loading cases...</div>
                        </div>
                        ...
                        <div class="person-custody-section">
                            <h4>Custody History 
                                <button class="btn btn-sm btn-primary" onclick="manageCustodyForPerson(${person.id}, '${fullName}'); return false;" style="float: right;">
                                    <i class="fas fa-edit"></i> Manage Custody
                                </button>
                            </h4>
                            <div id="personCustodyList">Loading custody records...</div>
                        </div>
                    </div>
...
async function loadPersonCasesInModal(personId) {
    try {
        ...
        if (response.status === 'success' && response.data.length > 0) {
            ...
            $('#personCasesList').html('<p style="color: #6b7280;">No connected cases</p>');
        }
    } catch (error) {
        $('#personCasesList').html('<p style="color: #ef4444;">Failed to load cases</p>');
    }
}
...
async function loadPersonCustodyInModal(personId) {
    try {
        ...
        if (response.status === 'success' && response.data.length > 0) {
            ...
            $('#personCustodyList').html('<p style="color: #6b7280;">No custody records</p>');
        }
    } catch (error) {
        $('#personCustodyList').html('<p style="color: #ef4444;">Failed to load custody records</p>');
    }
}
    ...
    try {
        ...
        if (response.status === 'success') {
            ...
            const result = await Swal.fire({
                ...
                width: '600px',
                showCancelButton: true,
                confirmButtonText: 'Update Person',
            ...
            if (result.isConfirmed) {
                showLoading('Updating person...');
                const updateResponse = await obAPI.updatePerson(personId, result.value);
                closeAlert();
                
                if (updateResponse.status === 'success') {
                    await showSuccess('Success!', 'Person details updated successfully');
        ...
        closeAlert();
        await showError('Error', 'Failed to update person: ' + error.message);
    }
}
// ============================================
...
async function manageCustodyForPerson(personId, personName) {
    try {
        showLoading('Loading custody information...');
        ...
        const result = await Swal.fire({
            title: `Manage Custody - ${personName}`,
            html: `
                <form id="manageCustodyForm" style="text-align: left;">
                    <div class="form-group">
                        ...
                        <select id="custody_action" class="swal2-input" onchange="toggleCustodyActionFields()" required>
                            ...
                            ${!activeCustody ? '<option value="create_arrested">Mark as Arrested (In Custody)</option>' : ''}
                            ${!activeCustody ? '<option value="create_bailed">Mark as Bailed</option>' : ''}
                            ${activeCustody && activeCustody.custody_status === 'in_custody' ? '<option value="update_to_bailed">Grant Bail</option>' : ''}
                            ${activeCustody && activeCustody.custody_status === 'in_custody' ? '<option value="update_to_released">Release from Custody</option>' : ''}
                        </select>
                    </div>
                    ...
                    <div id="arrested_custody_fields" style="display: none;">
                        <div class="form-group">
                            <label>Custody Location</label>
                            <input type="text" id="custody_location" class="swal2-input" value="Station Lock-up">
                        </div>
                        <div class="form-group">
                            <label>Cell Number</label>
                            <input type="text" id="cell_number" class="swal2-input" placeholder="e.g., Cell 3">
                        </div>
                        <div class="form-group">
                            <label>Custody Notes</label>
                            <textarea id="custody_notes" class="swal2-textarea" placeholder="Any notes..."></textarea>
                        </div>
                    </div>
                    
                    <!-- Bailed Fields -->
                    <div id="bailed_custody_fields" style="display: none;">
                        <h5 style="margin: 15px 0 10px 0; color: #3b82f6;">Bailer Information</h5>
                        <div class="form-group">
                            <label>Bailer Full Name *</label>
                            <input type="text" id="bailer_name" class="swal2-input" placeholder="Full name">
                        </div>
                        <div class="form-group">
                            <label>Bailer Phone *</label>
                            <input type="tel" id="bailer_phone" class="swal2-input" placeholder="+252...">
                        </div>
                        <div class="form-group">
                            <label>Bailer National ID</label>
                        ...
                        <div class="form-group">
                            <label>Relationship to Accused</label>
                            <input type="text" id="bailer_relationship" class="swal2-input" placeholder="e.g., Father, Brother">
                        </div>
                        <div class="form-group">
                            <label>Bailer Address</label>
                        ...
                        <div class="form-group">
                            <label>Bail Conditions</label>
                            <textarea id="bail_conditions" class="swal2-textarea" placeholder="Enter bail conditions..."></textarea>
                        </div>
                        <div class="form-group">
                            <label>Bail Amount</label>
                            <input type="number" id="bail_amount" class="swal2-input" placeholder="Amount">
                        </div>
                    </div>
                </form>
            `,
            width: '600px',
            showCancelButton: true,
            confirmButtonText: 'Update Custody',
            cancelButtonText: 'Cancel',
            didOpen: async () => {
                // Load person's cases
                ...
                if (casesResponse.data.length > 0) {
                    ...
                    caseSelect.innerHTML = '<option value="">No cases found</option>';
                }
            },
            preConfirm: () => {
                ...
                if (action === 'create_bailed' || action === 'update_to_bailed') {
                    ...
                    if (!bailerName || !bailerPhone) {
                        Swal.showValidationMessage('Bailer name and phone are required');
                        return false;
                    }
                }
        ...
        closeAlert();
        await showError('Error', 'Failed to manage custody: ' + error.message);
    }
}
