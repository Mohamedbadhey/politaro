 ...
 * This is ONLY for investigators
 */
async function showFullCaseDetailsModal(caseId) {
    try {
        ...
        // Only allow investigators to use this full modal
        ...
        if (response.status === 'success') {
            ...
            const modalHtml = `
                <div id="fullCaseModal" class="full-page-modal">
                    ...
                    ${buildCourtWorkflowActions(caseData)}
 ...
 */
function buildCourtWorkflowActions(caseData) {
    // Can take action if:
    // 1. Case is investigating/assigned (can close or send to court)
    // 2. Case is closed but not sent to court yet (can send to court)
    // 3. Case is court assigned back (can close)
    ...
    const hasOtherActions = canClose || canSendToCourt;
        ...
        <div class="court-workflow-bar" style="padding: 15px 20px; background: #f8f9fa; border-bottom: 2px solid #dee2e6;">
            ...
            <div style="display: flex; gap: 10px; align-items: center; flex-wrap: wrap; margin-top: ${courtStatusBadge || courtAssignmentAlert ? '10px' : '0'};">
                <div style="flex: 1; min-width: 200px;">
                    <span style="color: #6c757d;"><i class="fas fa-info-circle"></i> ${hasOtherActions ? 'Case Actions Available' : 'Case Report'}</span>
