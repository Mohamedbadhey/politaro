...
async function generateCustomReport(caseId, selectedSections, selectedPersons = []) {
    try {
        ...
        await Swal.fire({
            icon: 'success',
            title: 'Success',
            text: 'Customized report generated successfully!',
            timer: 2000,
...
function generateSectionContent(sectionKey, reportData, formatDate, escapeHtml) {
    ...
    switch (sectionKey) {
        ...
        case 'parties':
            ...
            if (accused.length > 0) {
                partiesHTML += window.generateAccusedSection(accused, witnesses, evidence);
            }
            
            if (accusers.length > 0) {
                partiesHTML += window.generateAccusersSection(accusers, witnesses, evidence);
            }
            
            if (neutralWitnesses.length > 0) {
                partiesHTML += window.generateNeutralWitnessesSection(neutralWitnesses);
            }
        ...
        case 'evidence':
            // Use enhanced evidence display with images
            return generateEnhancedEvidenceSection(crimeSceneEvidence, formatDate, escapeHtml);
            
        case 'timeline':
            // Use the EXACT function from full report  
            return window.generateTimelineSection(history);
            
        case 'conclusions':
            // Use the EXACT function from full report
            if (conclusions && conclusions.length > 0) {
                return window.generateConclusionsSection(conclusions[0]);
            } else {
