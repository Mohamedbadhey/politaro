 ...
 */
function loadIncidentEntryPage() {
    const html = `
                ...
                <form id="incidentEntryForm">
                    ...
                    <div class="form-section">
                        ...
                        <div class="form-row">
                            ...
                            <div class="form-group col-md-6">
                                ...
                                <input type="text" class="form-control" id="incident_location" name="incident_location" required placeholder="e.g., Main Street, City Center">
                            </div>
                        </div>
                        ...
                        <div class="form-row">
                            <div class="form-group col-md-6">
                                ...
                                <input type="text" class="form-control" id="crime_type" name="crime_type" required 
                                    placeholder="e.g., Traffic Accident, Lost Property, Public Disturbance">
                            </div>
                    ...
                    <div class="form-actions">
                        <button type="submit" name="action" value="submit" class="btn btn-primary" id="submitBtn">
                            <i class="fas fa-paper-plane"></i> <span data-i18n="submit_for_approval">Submit for Approval</span>
                        </button>
    ...
    $('#party_photo').on('change', function(e) {
        ...
        if (file) {
            ...
            if (file.size > 5 * 1024 * 1024) {
                showToast('error', 'Photo size must be less than 5MB');
                $(this).val('');
                return;
            }
            ...
            if (!file.type.match('image.*')) {
                showToast('error', 'Please select an image file (JPG/PNG)');
                $(this).val('');
                return;
            }
    ...
    try {
        ...
        if (hasPhoto) {
            ...
            if (!$('#party_role').val() || !$('#party_first_name').val() || !$('#party_last_name').val()) {
                showToast('error', 'Please fill all required party fields (Role, First Name, Last Name)');
            ...
            if (response.status === 'success') {
                ...
                showToast('error', response.message || 'Failed to create incident report');
            ...
            if (data.incident_description.length < 10) {
                showToast('error', 'Incident description must be at least 10 characters');
                $('#submitBtn, #draftBtn').prop('disabled', false);
                submitBtn.html(originalText);
                return;
            }
