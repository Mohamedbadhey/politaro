    ...
    public function generateFullReport($caseId = null)
    {
        ...
        try {
            ...
            foreach ($parties as &$party) {
                ...
                // Get investigation notes about this person
                $party['investigation_notes'] = $db->table('investigation_notes')
                    ->select('investigation_notes.*, users.full_name as investigator_name, users.badge_number')
                ...
                // Get witnesses supporting this person (if accused or accuser)
                if ($party['party_role'] === 'accused' || $party['party_role'] === 'accuser') {
                    ...
                    foreach ($party['supporting_witnesses'] as &$witness) {
                        ...
                        // Get investigation notes for this witness
                ...
                // Get witness affiliation if this is a witness
                if ($party['party_role'] === 'witness') {
                    $affiliatedPersonId = $party['affiliated_person_id'];
            ...
            // Get case assignments
            $assignments = $db->table('case_assignments')
                ->select('case_assignments.*, users.full_name as investigator_name, users.badge_number')
                ->join('users', 'users.id = case_assignments.investigator_id')
                ->where('case_assignments.case_id', $caseId)
                ->get()
                ->getResultArray();
            ...
            // Get created by user
            $createdBy = $db->table('users')
                ->select('full_name, email, phone, badge_number')
                ->where('id', $case['created_by'])
                ->get()
                ->getRowArray();
            ...
            try {
                $conclusions = $db->table('investigator_conclusions')
                    ->select('investigator_conclusions.*, users.full_name as investigator_name, users.badge_number')
